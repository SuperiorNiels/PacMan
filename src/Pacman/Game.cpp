...
namespace Pacman
{
    ...
    void Game::init()
    {
        ...
        Game::render = factory->createRenderSystem(world, config->getScreen_x(), config->getScreen_y(), timer, Game::events);
        ...
        manager->registerSystem(new MovementSystem(world, Game::events));
    ...
    void Game::checkEvents()
    {
        ...
        if (events->find(RESET) != events->end())
        {
            ...
            paused = true;
            Game::player->removeComponentByType(DEATH_COMPONENT);
            Game::player->addComponent(pause_text);
            manager->updateEntities();
            manager->updateSystems();
        }
