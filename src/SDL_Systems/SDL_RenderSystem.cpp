...
namespace SDL_Systems
{
    ...
    SDL_RenderSystem::SDL_RenderSystem(World *world, int screen_width, int screen_height, Systems::TimerSystem *timer, std::map<events_numbers, bool> *events) : RenderSystem(world, screen_width, screen_height, timer, events)
    {
        // Initialze SDL
        ...
        if (e->hasComponentType(DEATH_COMPONENT))
        {
            if (!rc->death_clips.empty())
            {
                if (rc->death_clips.size() != 1)
                {
                    ...
                    {
                        rc->current_frame = (rc->current_frame + 1) % rc->death_clips.size();
                        rc->count = 0;
                    }
                    else
                        rc->count++;

                    if (rc->current_frame == rc->death_clips.size() - 1)
                    {
                        (*events)[RESET] = true;
                    }

                    return *rc->death_clips[rc->current_frame];
                }
            }
        }
