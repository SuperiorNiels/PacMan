...
RenderSystem *
SDL_Factory::createRenderSystem(World *world, int screen_width, int screen_height, TimerSystem *timer, std::map<events_numbers, bool> *events)
{
    auto *res = new SDL_RenderSystem(world, screen_width, screen_height, timer, events);
    clearTextures();
    renderSystem = res;
    return res;
}
...
RenderComponent *SDL_Factory::createRenderComponent(std::string path, std::vector<clip> clips, std::vector<clip> death_clips)
{
    ...
    std::vector<SDL_Rect *> sdl_death_clips = std::vector<SDL_Rect *>();
    for (auto clip : death_clips)
    {
        auto *rect = new SDL_Rect();
        rect->x = clip.x;
        rect->y = clip.y;
        rect->w = clip.w;
        rect->h = clip.h;
        sdl_death_clips.push_back(rect);
    }
    to_return->death_clips = std::move(sdl_death_clips);
